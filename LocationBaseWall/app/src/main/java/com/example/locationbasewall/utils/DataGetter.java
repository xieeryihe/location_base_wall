...
public class DataGetter {
    ...
    public static int getLocationAndPostOverviewData
            (Activity activity,PostAdapter postAdapter,
             String key_words, int page_num, int page_size, int distance){
        ...
        location.getCurrentLocation(new Location.LocationCallback() {
            ...
            public void onLocationReceived(double latitude, double longitude, String province, String city, String address) {
                String targetUrl = String.format("" +
                                ...
                                "page_num=%d&page_size=%d&key_words=%s&location_x=%.2f&location_y=%.2f&distance=%d",
                        page_num, page_size, key_words,longitude, latitude, distance);

                System.out.println("主页访问的链接是");
    ...
    public static int processPostOverviewData(JSONObject data, ArrayList<Post> postList) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                Post post = new Post(id, user_id, username,user_picture, title, text, -1, "",date, location_x, location_y, ip_address, distance);
                postList.add(post);
            }
